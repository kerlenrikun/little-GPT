...
class _ArticleContentPageState extends State<ArticleContentPage> {
  ...
  final ArticleDbManager articleDao = ArticleDbManager(); // ✅ 数据库实例

  String? articleId; // ✅ 当前文章ID
  OverlayEntry? _overlayEntry;
  Offset? _lastGlobalTap;
  ...
  void initState() {
    ...
    articleId = articleInfoProvider.articleId; // ✅ 从 Provider 拿文章 ID
    _loadFromLocalDb(); // ✅ 加载本地收藏与评论
  ...
  void _updateTitle() {
    setState(() {
      articleText = articleInfoProvider.articleContent;
      articleId = articleInfoProvider.articleId;
    });
  }
  ...
  Future<void> _loadFromLocalDb() async {
    ...
    // ✅ 读取高亮与评论
    final hData = await articleDao.getHighlights(articleId!);
    final cData = await articleDao.getComments(articleId!);
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  tooltip: '收藏',
                  onPressed: () async {
                    if (currentSelection != null && articleId != null) {
                      ...
                      // ✅ 保存到数据库
                      await articleDao.insertHighlight({
  ...
  Widget _buildRichTextWithHighlights() {
    ...
    for (final m in merged) {
      ...
      if (m.isHighlight && m.isComment) {
        bg = Colors.purpleAccent.withOpacity(0.35);
      } else if (m.isHighlight) {
        bg = Colors.blueAccent.withOpacity(0.3);
      } else if (m.isComment) {
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (text.isNotEmpty && articleId != null) {
                  setState(() {
                    comments.add(
                      CommentData(
                        start: start,
                  ...
                  // ✅ 保存评论到数据库
                  await articleDao.insertComment({
                    'article_id': articleId,
                    'start': start,
                    'end': end,
                    'text': selectedText,
...
List<_RangeMark> _mergeRanges(
  ...
  for (int i = 0; i < sorted.length - 1; i++) {
    ...
    result.add(mark);
    if (debug) debugPrint('DEBUG _RangeMark: $mark');
  }
  return result;
}
